...
const GamePad = ({ gamepadSocketAddress, gamepadSocketPort }) => {
  const [ isRequestedAnimationFrame, setIsRequestedAnimationFrame ] = useState(false);

  function gamepadListener (socketGamepad) {
    if (!isRequestedAnimationFrame) {
      setIsRequestedAnimationFrame(true);
    }
  ...
  useEffect(() => {
    const socketGamepad = io.connect(`http://${gamepadSocketAddress}:${gamepadSocketPort}`);
